...
async function getStudyList(callback) {
    const patients = await callAPI('http://breastscreening.isr.tecnico.ulisboa.pt:4550/patients?expand')
    const p = patients.reduce((prev, next) => {
        return prev.concat(next.Studies);
    }, []).map(studyId => {
        const url = 'http://breastscreening.isr.tecnico.ulisboa.pt:4550/studies/' + studyId + '?';
